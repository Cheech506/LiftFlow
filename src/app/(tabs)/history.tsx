...
export default function HistoryScreen() {
  ...
  const filteredWorkouts = useMemo(() => {
    ...
    return completedWorkouts
      .filter((item) => {
        if (templateId && item.sourceTemplateId !== templateId) return false;
        if (source === 'strong' && item.importSource !== 'strong') return false;
        if (source === 'liftflow' && item.importSource === 'strong') return false;
        if (!normalizedQuery) return true;
        const searchable = [
          item.name,
          item.notes,
          item.sourceFolder,
          ...item.exercises.flatMap((exercise) => [exercise.name, exercise.notes]),
        ]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
  ...
  }, [completedWorkouts, query, source, templateId]);
    ...
    <>
      {view === 'timeline' ? (
        <FlatList
          ...
          renderItem={({ item }) => (
            <WorkoutHistoryCard workout={item} onPress={() => setSelectedWorkoutId(item.id)} />
          )}
